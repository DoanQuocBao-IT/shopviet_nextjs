...
const EditProduct = ({ initialValues, setVisibleChange, fetchProducts }) => {
  const onSubmit = (data) => {
  ...
  const handleEditProduct = async (data) => {
    try {
      const response = await apiInstance.put(
        `/seller/product/${initialValues.id}`,
        data
      )
  ...
  const [productImage, setProductImage] = useState(initialValues.image)
