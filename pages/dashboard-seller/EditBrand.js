...
const EditBrand = ({ id, image, name, setVisibleChange, fetchProducts }) => {
  ...
  const handleEditBrand = async (data) => {
    setLoading(true)
    try {
      const response = await apiInstance.put(`/seller/brand/${id}`, data)
      if (response.status == 200) {
        showToast('success', 'Edit brand success', '')
